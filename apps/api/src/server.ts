...
export async function buildServer(): Promise<FastifyInstance> {
  ...
  const app = Fastify({
    ...
    // TODO(SF-003): revert pluginTimeout to default 10_000 once Redis
    // cold-start ECONNRESET on Railway is resolved (incident 2026-05-24).
    // No longer load-bearing once WS gateway uses non-blocking subscribe
    // (see analytics.gateway.ts).
