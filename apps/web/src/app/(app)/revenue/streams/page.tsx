...
export default function StreamBreakdownPage(): JSX.Element {
  ...
  const STREAMS: Array<{ key: 'BUZZPAY' | 'PIXIE' | 'MICAMP'; label: string; sub: string; href: string; tone: 'accent' | 'success' | 'warn' }> = [
    { key: 'BUZZPAY', label: 'BuzzPay', sub: 'Lender rev share', href: '/buzzpay', tone: 'accent' },
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="Stream breakdown" subtitle="The three streams · 90 days · click a card to drill in" />
          ...
          {STREAMS.map((s) => (
            <div key={s.key}>
              <div className="flex items-center justify-between mb-1.5 text-sm">
                <Link href={s.href} className="text-ink hover:text-accent font-medium">{s.label}</Link>
                <span className="numeric text-ink2">{formatMoney(totals[s.key] ?? 0)} <span className="text-muted">· {formatPct(grand ? (totals[s.key] ?? 0) / grand : 0)}</span></span>
              </div>
