...
export default function PropensityPage(): JSX.Element {
  ...
  const byLender = useQuery({
    queryKey: ['analytics.propensity-by-lender'],
    queryFn: () => api<LenderRow[]>('/analytics/propensity-by-lender'),
  });
  const trend = useQuery({
    queryKey: ['analytics.propensity-trend'],
    queryFn: () => api<TrendRow[]>('/analytics/propensity-trend'),
  });

  const rows = cal.data ?? [];
  const lenders = byLender.data ?? [];
  ...
  // Brier-ish calibration: weighted MAE between bucket midpoint and actual approval
  ...
  const trendDisplay = trendRows.map((r) => ({
    week: shortWeek(r.week),
    predicted: r.avgPredicted,
    actual: r.actualApproval,
    absError: r.absError,
  }));
    ...
    <div className="space-y-6">
      ...
      >
        ...
        <div className="flex items-center gap-5 px-3 pt-3 text-[11px] text-muted">
          ...
          <Legend color="#1D4ED8" label="Approval rate" />
      ...
      <SectionCard
        title="Calibration drift over time"
        subtitle="weekly mean absolute error · trend line should hug the floor — if it climbs, the model is decaying"
        bodyClassName="p-3"
      >
        <div style={{ height: 240 }}>
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={trendDisplay} margin={{ top: 10, right: 16, bottom: 8, left: 8 }}>
              <CartesianGrid stroke="#EEF1F5" vertical={false} />
        ...
        <div className="flex items-center gap-5 px-3 pt-3 text-[11px] text-muted">
          <Legend color="#94A3B8" label="Avg predicted" />
          <Legend color="#1D4ED8" label="Actual approval" />
          <Legend color="#0F172A" label="Absolute error" />
        </div>
      </SectionCard>

      <SectionCard
        title="Per-lender calibration"
        subtitle="how Pixie's propensity tracks each lender's actual approvals · ≥10 decisions"
        bodyClassName="p-0"
      >
          ...
          <table className="tbl">
            ...
            <tbody>
              {lenders.map((r) => {
                ...
                return (
                  <tr key={r.lenderName}>
                    <td className="text-ink font-medium">{r.lenderName}</td>
                    <td className="numeric text-right text-ink">{formatNumber(r.n)}</td>
                    <td className="numeric text-right text-muted">{formatPct(r.avgPredicted)}</td>
                    <td className="numeric text-right text-ink2">{formatPct(r.actualApproval)}</td>
                    <td className={`numeric text-right font-medium ${tone}`}>
                      {r.delta > 0 ? '+' : ''}
                      {(r.delta * 100).toFixed(1)}%
                    </td>
                    <td className={`text-xs ${tone}`}>{verdict}</td>
                  </tr>
                );
              })}
              {lenders.length === 0 && (
                <tr>
                  <td colSpan={6} className="text-muted py-6 text-center">
                    Need ≥10 decisions per lender for a reading.
                  </td>
                </tr>
              )}
            </tbody>
