...
const SECRETS = [
  { key: 'BUZZPAY_WEBHOOK_SECRET', label: 'BuzzPay HMAC secret', status: 'PROCESSED' },
...
export default function SecretsPage(): JSX.Element {
  return (
    <div className="space-y-6">
      ...
      <SectionCard title="Inventory" subtitle="loaded from .env at boot · validated by Zod" bodyClassName="p-0">
        <div className="overflow-x-auto">
          <table className="tbl">
            ...
            <tbody>
              {SECRETS.map((s) => (
                <tr key={s.key}>
                  <td className="numeric"><code className="kbd">{s.key}</code></td>
      ...
      <SectionCard title="Rotation playbook" bodyClassName="p-5">
        <ol className="list-decimal list-inside text-sm text-ink2 space-y-1">
          ...
          <li>For HMAC webhook secrets: coordinate the cutover window with the source vendor. Update <code className="kbd">.env</code>, redeploy.</li>
          <li>For PII keys: register the new key version in <code className="kbd">encryption.ts</code>; do not retire the old version while ciphertext exists. Backfill is a separate v1.1 procedure.</li>
          <li>For JWT secrets: rotation invalidates all sessions. Plan a maintenance window or implement RS256 + KMS first (v1.1).</li>
        </ol>
      </SectionCard>
    </div>
  );
