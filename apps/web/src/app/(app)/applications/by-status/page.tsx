...
export default function AppsByStatusPage(): JSX.Element {
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="Applications · by status" subtitle="The funnel sliced into queues · each column is a state" />

      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
        {STATUSES.map((s, i) => (
          <KpiCard key={s} label={s} value={(queries[i]!.data?.data?.length ?? 0).toString()} hint="last 20 in state" />
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {STATUSES.map((s, i) => {
          ...
          return (
            <SectionCard key={s} title={s} subtitle={`${rows.length} application${rows.length === 1 ? '' : 's'}`} bodyClassName="p-0">
                ...
                {rows.map((a) => (
                  <li key={a.id} className="px-4 py-2.5 hover:bg-paper/60">
                    <Link href={`/applications/${a.id}`} className="block">
                      ...
                      <div className="mt-1 text-[11px] text-muted">{a.consumerNameMasked} · {a.consumerEmailMasked}</div>
                      <div className="mt-1 text-[11px] text-muted numeric">credit {a.creditScore ?? '—'} · {formatDateTime(a.createdAt)}</div>
                    </Link>
                  </li>
                ))}
                {rows.length === 0 && <li className="px-4 py-6 text-center text-muted text-sm">empty</li>}
              </ul>
            </SectionCard>
          );
        })}
