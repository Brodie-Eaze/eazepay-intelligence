  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <KpiCard label="Our share" value={formatMoney(totalFees)} hint="50% of gross fees reported" />
        <KpiCard label="Reversals" value={reversals.length.toString()} hint={`${formatMoney(totalReversals)} netted`} />
        <KpiCard label="Transactions" value={totalTxns.toLocaleString('en-AU')} hint="processed across the network" />
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="tbl">
            ...
            <tbody>
              {rows.map((r) => {
                ...
                return (
                  <tr key={r.idempotencyKey}>
                    ...
                    <td className="numeric text-right text-ink2">{r.metadata.gross ? formatMoney(r.metadata.gross) : '—'}</td>
              ...
              {rows.length === 0 && <tr><td colSpan={7} className="text-muted py-8 text-center">No MiCamp activity yet.</td></tr>}
            </tbody>
          </table>
        </div>
      </SectionCard>
