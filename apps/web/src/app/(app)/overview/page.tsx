...
export default function OverviewPage(): JSX.Element {
  ...
  const pixie = useQuery({
    queryKey: ['pixie.breakpoint'],
  ...
  const customers = useQuery({
    queryKey: ['customers.book.preview'],
    queryFn: () => api<CustomerRow[]>('/customers?limit=8'),
  });
  const live = useQuery({
  ...
  const dayLabel = new Date().toLocaleDateString('en-AU', {
    weekday: 'long',
    day: 'numeric',
    month: 'long',
  });
  ...
  const subprimeApps =
    ((risk.data ?? []).find((r) => r.riskBand === 'SUBPRIME')?.count ?? 0) +
  ...
  return (
    <div className="space-y-6">
      <NarrativeHero
        ...
        narrative={
          <>
            <span className="text-accent">{formatNumber(submittedCount)} applications</span> moved
            through the network. <span className="text-accent">{formatPct(o.approvalRate)}</span>{' '}
            approved, <span className="text-accent">{formatMoney(o.totalRevenue)}</span> booked.
          </>
        }
        subtext={`${greeting}. HighSale pre-qual feeds the decision engine; MedPay / TradePay / CoachPay book the loans; MiCamp clears the rails.`}
        kpis={[
          { label: 'Total revenue', value: formatMoney(o.totalRevenue), hint: `MoM ${delta.text}` },
          {
            label: 'Approval rate',
            value: formatPct(o.approvalRate),
            hint: `${formatNumber(submittedCount)} submitted`,
          },
          {
            label: 'Funding rate',
            value: formatPct(o.fundingRate),
            hint: `${formatNumber(fundedCount)} funded`,
          },
          {
            label: 'Active partners',
            value: formatNumber(o.activePartnerCount),
            hint: 'last 30 days',
          },
          {
            label: 'Pixie pulls 24h',
            value: formatNumber(o.pixiePullsLast24h),
            hint: pixie.data?.aboveBreakpoint ? 'above breakpoint' : 'below breakpoint',
          },
        ]}
      />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        >
          {revenue.data ? (
            <RevenueAreaChart data={revenue.data} height={300} />
          ) : (
        ...
        <SectionCard
          ...
          action={
            <Link href="/risk" className="text-xs text-accent hover:underline">
              Risk profiles →
            </Link>
          }
        >
          <div className="space-y-3">
            ...
            />
            <BandRow
              label="Near-prime 660–719"
              count={(risk.data ?? []).find((r) => r.riskBand === 'NEAR_PRIME')?.count ?? 0}
              total={totalApps}
              band="NEAR_PRIME"
              tone="accent"
            />
            <BandRow
              label="Subprime 580–659"
              count={(risk.data ?? []).find((r) => r.riskBand === 'SUBPRIME')?.count ?? 0}
              total={totalApps}
              band="SUBPRIME"
              tone="warn"
            />
            <BandRow
              label="Deep subprime < 580"
              count={(risk.data ?? []).find((r) => r.riskBand === 'DEEP_SUBPRIME')?.count ?? 0}
              total={totalApps}
              band="DEEP_SUBPRIME"
              tone="danger"
            />
            <div className="pt-3 border-t border-line2 text-[11px] text-muted">
              {formatNumber(unscored)} of {formatNumber(totalApps)} unscored (
              {formatPct(totalApps ? unscored / totalApps : 0)})
            </div>
          </div>
        </SectionCard>
      </div>
      ...
      <div className="card divide-x divide-line2 grid grid-cols-2 md:grid-cols-4">
        ...
        <Stat
          ...
          hint={`${formatNumber(fundedCount)} funded · ${formatPct(o.fundingRate)} fund rate`}
        />
        <Stat
          label="Active partners"
          value={formatNumber(o.activePartnerCount)}
              ...
              {(customers.data ?? []).slice(0, 8).map((c) => (
                <tr key={c.emailHash}>
                  ...
                  <td className="numeric text-muted text-xs whitespace-nowrap">
                    {new Date(c.latestApplicationAt).toLocaleDateString('en-AU', {
                      day: 'numeric',
                      month: 'short',
                    })}
                  </td>
    ...
    <div>
      <div className="flex items-center justify-between text-xs mb-1.5">
        ...
        <span className="numeric text-ink2">
          {formatNumber(count)} <span className="text-muted">· {formatPct(pct, 0)}</span>
        </span>
      </div>
      <MiniBar
        value={pct}
...
function Stat({
  ...
  return (
    <div className="px-5 py-4">
      ...
      <div className={`numeric text-2xl font-semibold mt-1 tracking-tight ${toneClass}`}>
        {value}
      </div>
    ...
    <div className="px-5 py-4">
      ...
      <div className="flex items-baseline gap-2 mt-1">
        <span className="numeric text-2xl font-semibold text-ink tracking-tight">
          {formatNumber(data.collectiveLast24h)}
        </span>
        <span className="text-[11px] text-muted numeric">/ {formatNumber(data.threshold)}</span>
      </div>
      <MiniBar
        className="mt-2 h-1.5"
        tone={data.aboveBreakpoint ? 'success' : 'warn'}
        value={ratio}
      />
      <div
        className={`text-[11px] mt-1.5 font-medium ${data.aboveBreakpoint ? 'text-success' : 'text-warn'}`}
      >
...
function Skeleton(): JSX.Element {
  return (
    <div className="space-y-6">
      <div
        className="h-44 rounded-2xl bg-surface animate-pulse"
