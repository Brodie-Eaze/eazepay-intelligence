...
| **Elevation of privilege** between scopes               | RBAC checked at the route level (`requireRole`, `denyInvestorScope`); investor responses produced by separate response schemas; cookies are `HttpOnly` + `Secure` + `SameSite=None` in prod (cross-subdomain on `*.up.railway.app`, see `apps/api/src/shared/utils/cookies.ts:42`) and `SameSite=Strict` in dev; OAuth state cookies override to `Lax`. `__Host-` cookie prefix invariants (Secure + no Domain + Path=/) enforced in helper. |
