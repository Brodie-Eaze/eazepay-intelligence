...
- [~] **2.8** HighSale enrichment ingestion — per-application credit-data snapshots (~70 fields across 10 logical blocks) pushed to `/integration/highsale/snapshots`. **Scaffold landed _(this session)_**: HMAC-verified route stub + fully-typed Zod envelope against the real HighSale JSON sample + dbt staging models (one mainline, one protected-class-gated) + `HIGHSALE_WEBHOOK_SECRET`. **Pending**: `HighsaleSnapshot` Prisma model + migration; per-org-DEK encryption of `request_body` PII; `application_id ↔ transaction_id` correlation token threaded via App → HighSale → back; customer-detail UI tab; protected-class permission + audit trail. See `docs/architecture/data-warehouse-overview.md` § Plane 2.
