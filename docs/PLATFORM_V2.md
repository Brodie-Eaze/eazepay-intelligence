...
| 5       | 2026-05-08 | _(this session)_                | **1.5 rotateDek** — generates new DEK + deactivates prior version atomically, returns the new keyId. **1.6** — `/platform/orgs/:id/rotate-dek` (SUPER), `/platform/health` (STAFF) cross-tenant snapshot, `/platform/sessions` (STAFF) refresh-token inventory. All audited. **+3 tests, 112 passing.**                                                       |
