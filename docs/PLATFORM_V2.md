...
| 6       | 2026-05-08 | _(this session)_                | **1.5 AwsKmsClient** — production driver via `@aws-sdk/client-kms` (ap-southeast-2 default). **KMS factory** — env-driven selection (`KMS_DRIVER` override → NODE_ENV-based default). Bootstrap seed routed through factory. **End-to-end encryption integration test** on real Postgres: round-trip, cross-org tampering negative test, rotate-then-decrypt-old-ciphertext (proves deactivated DEKs remain readable). **+11 tests, 123 passing.** |
