...
- [~] **2.8** HighSale enrichment ingestion — per-application credit-data snapshots (12 data points) pushed to `/integration/highsale/snapshots`. **Scaffold landed _(this session)_**: HMAC-verified route stub + Zod envelope (4 of 12 fields typed; 8 in passthrough `rawPayload`) + dbt staging placeholder + `HIGHSALE_WEBHOOK_SECRET`. **Pending JSON spec from HighSale**: `HighsaleSnapshot` Prisma model + migration + per-org-DEK encryption of sensitive fields + drain into `applications` join key + customer-detail UI tab. See `docs/architecture/data-warehouse-overview.md` § Plane 2.
