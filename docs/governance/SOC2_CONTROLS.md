...
| TSC       | Control                                                                    | Status | Evidence                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| --------- | -------------------------------------------------------------------------- | ------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **CC6.1** | Implements logical access security software, infrastructure, architectures | ✅     | RBAC enforced in `shared/middleware/rbac.middleware.ts`. Roles: ADMIN / OPERATOR / INVESTOR / VIEWER. Roles + scopes documented in `docs/GLOSSARY.md`. Cookie-based session: `HttpOnly` + `Secure` always; `SameSite=None` in production (web + API on sibling `*.up.railway.app` subdomains are cross-site to the browser, so `Strict`/`Lax` would drop the cookie on credentialed XHR and break auth — CSRF is covered by the double-submit token, see CC6.6) and `SameSite=Strict` in dev. OAuth state cookies override to `Lax` so they survive the top-level redirect from the IdP. Single source of truth: `apps/api/src/shared/utils/cookies.ts:42`. |
...
| -------------------------------------------------------------------------------------- | ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **PI1.1** Inputs are complete, accurate, valid, authorized                             | ✅     | All ingress is Zod-validated; webhook HMAC + timestamp + idempotency-key; PAT scope gates on every write. See Appendix A.                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| **PI1.2** System processing is complete, valid, accurate, timely, authorized           | ✅     | Transactional outbox guarantees side-effect delivery is atomic with business writes (`schema.prisma` `OutboxEvent` + `outbox.worker.ts`). The sweeper's cross-tenant RLS carve-out is explicitly designed and reviewed: see [`docs/architecture/adr/ADR-003-outbox-sweeper-rls-carveout.md`](../architecture/adr/ADR-003-outbox-sweeper-rls-carveout.md). Authorization is no longer implicit — the ADR enumerates 10 compensating controls and a quarterly review cadence. Closes finding **PI-019**. |
| **PI1.3** Output is complete, accurate, distributed to meet objectives                 | ✅     | Outbound webhooks HMAC-signed; delivery worker retries with backoff; DLQ on `OUTBOX_MAX_ATTEMPTS` exceeded, operator-visible at `POST /platform/outbox/dlq/:id/replay`.                                                                                                                                                                                                                                                                                                                                |
| **PI1.4** Inputs / outputs are stored completely, accurately, timely                   | ✅     | Append-only `audit_logs` + `outbox_events` (UPDATE/DELETE REVOKEd at role level on ledger tables). Replication lag exposed at `/health/ready`.                                                                                                                                                                                                                                                                                                                                                          |
| **PI1.5** System processing activities are authorized to meet objectives               | ✅     | RBAC + PAT scopes on every state-changing route; sweeper carve-out documented in ADR-003 with explicit compensating controls and quarterly review (next: 2026-08-31).                                                                                                                                                                                                                                                                                                                                  |
...
| Cookie flags (`HttpOnly` + `Secure` + `SameSite=None` in prod / `Strict` in dev) | `apps/api/src/shared/utils/cookies.ts:42`                                            |
